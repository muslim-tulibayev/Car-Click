...
class HomeLayer
{
    ...
    {
        // * Gate
        if (!$update->tg_chat->user) return;
        if (!$update->tg_chat->action) return;
    ...
    private static function keywords($update)
    {
        switch ($update->data) {
            case trans('msg.my_cars_btn'):
                self::myCars($update);
                return true;
            case trans('msg.add_car_btn'):
                self::addYourCar($update);
                return true;
            default:
                return false;
        }
    ...
    {
        $cars = $update->tg_chat->user->cars()->get();
        if (!count($cars))
    ...
    private static function addYourCar($update)
    {
        $update->tg_chat->update(["action" => 'home>car>adding>waiting_company']);
        return $update->bot->sendMessage([
            'chat_id' => $update->chat_id,
            'reply_markup' => KeyboardLayout::cancel(),
            'text' => trans('msg.ask_car_company'),
        ]);
    }
}
