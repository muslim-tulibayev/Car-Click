@props([
    ...
    'langs_list' => App\Models\UserChat::langsList(),
])
...
<x-layouts.app>
    ...
    <form action="{{ route('userchats.update', ['userchat' => $userchat->id]) }}" method="POST"
            ...
            <x-card-item name="Chat id (in Tg)">
                <div>
                    <input type="text" name="chat_id"
                        ...
                        value="{{ old('chat_id') ?? $userchat->chat_id }}" />
                </div>
            </x-card-item>
                        ...
                        @foreach ($users_list as $user)
                            <option value="{{ $user->id }}"
                                @if (old('user_id')) @if (old('user_id') == $user->id)
                                        selected @endif
                            @else @if ($userchat->user_id === $user->id) selected @endif @endif>
            ...
            <x-card-item name="Action">
                <div>
                    <input type="text" name="action"
                        class="w-[150px] text-sm text-gray-400 outline-none bg-gray-100 focus:bg-gray-300 rounded-md px-[3px]"
                        value="{{ old('action') ?? $userchat->action }}" />
                </div>
            </x-card-item>
                ...
                <div class="relative inline-flex">
                    ...
                    <select name="lang"
                        ...
                        @foreach ($langs_list as $lang)
                            ...
                            @else @if ($userchat->lang === $lang) selected @endif @endif>
                                {{ $lang }}
                            </option>
                        @endforeach
                    </select>
