@props([
    ...
    'langs_list' => App\Models\DealerChat::langsList(),
])
...
<x-layouts.app>
    ...
    <form action="{{ route('dealerchats.update', ['dealerchat' => $dealerchat->id]) }}" method="POST"
            ...
            <x-card-item name="Chat id (in Tg)">
                <div>
                    <input type="text" name="chat_id"
                        ...
                        value="{{ old('chat_id') ?? $dealerchat->chat_id }}" />
                </div>
            </x-card-item>
                        ...
                        @foreach ($dealers_list as $dealer)
                            <option value="{{ $dealer->id }}"
                                @if (old('dealer_id')) @if (old('dealer_id') == $dealer->id)
                                        selected @endif
                            @else @if ($dealerchat->dealer_id === $dealer->id) selected @endif @endif>
            ...
            <x-card-item name="Action">
                <div>
                    <input type="text" name="action"
                        class="w-[150px] text-sm text-gray-400 outline-none bg-gray-100 focus:bg-gray-300 rounded-md px-[3px]"
                        value="{{ old('action') ?? $dealerchat->action }}" />
                </div>
            </x-card-item>
            ...
            <x-card-item name="Data">
                <div>
                    <input type="text" name="data"
                        class="w-[150px] text-sm text-gray-400 outline-none bg-gray-100 focus:bg-gray-300 rounded-md px-[3px]"
                        value="{{ old('data') ?? $dealerchat->data }}" />
                </div>
            </x-card-item>
            <x-v-error name="data" />
                ...
                <div class="relative inline-flex">
                    ...
                    <select name="lang"
                        ...
                        @foreach ($langs_list as $lang)
                            ...
                            @else @if ($dealerchat->lang === $lang) selected @endif @endif>
                                {{ $lang }}
                            </option>
                        @endforeach
                    </select>
